...
const runtimeKey = "starbase.runtime"
...
func (rt *Runtime) Call(ctx context.Context, name string, args []any, kwargs map[string]any) (starlark.Value, error) {
	...
	rt.mu.RUnlock()
	if !ok {
		return nil, fmt.Errorf("starbase: global %q not defined", name)
	}
	fn, ok := val.(starlark.Callable)
	if !ok {
		return nil, fmt.Errorf("starbase: global %q is not callable (type %s)", name, val.Type())
	}
...
func (rt *Runtime) CallFn(ctx context.Context, fn starlark.Callable, args []any, kwargs map[string]any) (starlark.Value, error) {
	if fn == nil {
		return nil, fmt.Errorf("starbase: CallFn: fn is nil")
	}
	pos, err := convertArgs(args)
	if err != nil {
		return nil, err
...
func convertArgs(in []any) (starlark.Tuple, error) {
	...
	for i, v := range in {
		...
		if err != nil {
			return nil, fmt.Errorf("starbase: arg %d: %w", i, err)
		}
		out = append(out, sv)
	}
	return out, nil
...
func convertKwargs(in map[string]any) ([]starlark.Tuple, error) {
	...
	for k, v := range in {
		sv, err := startype.Go[any](v).ToStarlarkValue()
		if err != nil {
			return nil, fmt.Errorf("starbase: kwarg %q: %w", k, err)
		}

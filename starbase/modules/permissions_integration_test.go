...
func TestFSModulePermissions(t *testing.T) {
	...
	// Load the fs module
	fsModule := &fs.Module{}
	exports, err := fsModule.Load(&starbase.ModuleConfig{})
	if err != nil {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !starbase.IsPermissionError(err) {
...
func TestOSModulePermissions(t *testing.T) {
	...
	exports, err := osModule.Load(&starbase.ModuleConfig{})
	if err != nil {
...
func TestHTTPModulePermissions(t *testing.T) {
	...
	exports, err := httpModule.Load(&starbase.ModuleConfig{})
	if err != nil {
...
func TestIOModulePermissions(t *testing.T) {
	...
	exports, err := ioModule.Load(&starbase.ModuleConfig{})
	if err != nil {
...
func TestModulePermissionsAllowed(t *testing.T) {
	...
	thread := &starlark.Thread{Name: "test"}
	checker, err := starbase.NewPermissionChecker(starbase.TrustedPermissions())
	if err != nil {
		t.Fatalf("NewPermissionChecker error: %v", err)
	}
	starbase.SetPermissions(thread, checker)
	...
	osModule := &osmod.Module{}
	_, err = osModule.Load(&starbase.ModuleConfig{})
	if err != nil {
		t.Fatalf("os.Load error: %v", err)
	}
...
func TestSelectivePermissions(t *testing.T) {
	...
	// Load modules
	osModule := &osmod.Module{}
	osModule.Load(&starbase.ModuleConfig{})
	aliases := osModule.Aliases()

	fsModule := &fs.Module{}
	fsExports, _ := fsModule.Load(&starbase.ModuleConfig{})

...
func (m *Module) sshConfig(thread *starlark.Thread, fn *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
	...
	if err := starbase.Check(thread, "ssh", "config", ""); err != nil {
		return nil, err
	}
...
func (c *SSHClient) Attr(name string) (starlark.Value, error) {
	...
	if baseName, ok := strings.CutPrefix(name, "try_"); ok {
		switch baseName {
		case "exec":
			return starbase.TryWrap("ssh.client."+name, starlark.NewBuiltin("ssh.client.exec", c.exec)), nil
		case "upload":
			return starbase.TryWrap("ssh.client."+name, starlark.NewBuiltin("ssh.client.upload", c.upload)), nil
		case "download":
			return starbase.TryWrap("ssh.client."+name, starlark.NewBuiltin("ssh.client.download", c.download)), nil
		}
		return nil, nil
	}
	switch name {

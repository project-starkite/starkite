...
func (t *Template) Attr(name string) (starlark.Value, error) {
	...
	if base, ok := strings.CutPrefix(name, "try_"); ok {
		if method := t.methodBuiltin(base); method != nil {
			return starbase.TryWrap("template."+name, method), nil
		}
		return nil, nil
	}
...
func parseTemplate(source, leftDelim, rightDelim string, thread *starlark.Thread, config *starbase.ModuleConfig) (*Template, error) {
	t := template.New("tmpl")

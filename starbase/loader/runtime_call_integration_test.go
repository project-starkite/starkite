...
func newRuntimeWithModules(t *testing.T, perms *starbase.PermissionConfig) *starbase.Runtime {
	t.Helper()
	reg := loader.NewDefaultRegistry(&starbase.ModuleConfig{})
...
func TestRuntime_Call_UsesStarbaseModule(t *testing.T) {
	...
	rt := newRuntimeWithModules(t, starbase.TrustedPermissions())
...
func TestRuntime_Call_RespectsPermissions(t *testing.T) {
	rt := newRuntimeWithModules(t, starbase.SandboxedPermissions())

...
kite run deploy-controller.star

# Override image and replicas
kite run deploy-controller.star --var image=myregistry/controller:v1.2.0 --var replicas=3
```
...
### Generating artifacts with `kite kube gen-controller-artifacts`

For teams that prefer static YAML, `kite` can generate manifests from a Starlark definition:
...
```bash
kite run configmap-sync.star
kite run configmap-sync.star --var namespace=my-ns
```

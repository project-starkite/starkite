...
func (m *Module) Load(config *libkite.ModuleConfig) (starlark.StringDict, error) {
	m.once.Do(func() {
		...
		members := starlark.StringDict{
			...
			"sh":         starlark.NewBuiltin("os.sh", m.sh),
			"bash":       starlark.NewBuiltin("os.bash", m.bash),
			"zsh":        starlark.NewBuiltin("os.zsh", m.zsh),
			"cmdexe":     starlark.NewBuiltin("os.cmdexe", m.cmdexe),
			"powershell": starlark.NewBuiltin("os.powershell", m.powershell),
		...
		m.aliases = starlark.StringDict{
			...
			"shell":      starlark.NewBuiltin("shell", m.shell),
			"sh":         starlark.NewBuiltin("sh", m.sh),
			"bash":       starlark.NewBuiltin("bash", m.bash),
			"zsh":        starlark.NewBuiltin("zsh", m.zsh),
			"cmdexe":     starlark.NewBuiltin("cmdexe", m.cmdexe),
			"powershell": starlark.NewBuiltin("powershell", m.powershell),
...
// runCmd parses args, checks permissions, and executes a command directly.

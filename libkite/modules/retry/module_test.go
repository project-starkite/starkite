...
func makeCounterFunc(failCount int, successVal starlark.Value) starlark.Callable {
	...
	return starlark.NewBuiltin("testfn", func(_ *starlark.Thread, _ *starlark.Builtin, _ starlark.Tuple, _ []starlark.Tuple) (starlark.Value, error) {
		count++
		if count <= failCount {
			return libkite.ResultErr("attempt failed"), nil
		}
		return libkite.ResultOK(successVal), nil
	})
}

func asRetryResult(t *testing.T, val starlark.Value) *libkite.RetryResult {
	t.Helper()
	rr, ok := val.(*libkite.RetryResult)
	if !ok {
...
func attrInt(t *testing.T, rr *libkite.RetryResult, name string) int {
	t.Helper()
	v, _ := rr.Attr(name)
...
func TestDoSuccessFirstAttempt(t *testing.T) {
	thread := testThread()
	fn := makeFunc(libkite.ResultOK(starlark.String("ok")))
...
func TestDoAllAttemptsFail(t *testing.T) {
	thread := testThread()
	fn := makeFunc(libkite.ResultErr("always fails"))
...
func TestDoMaxAttempts(t *testing.T) {
	thread := testThread()
	fn := makeFunc(libkite.ResultErr("fail"))
...
func TestDoRetryOnPredicate(t *testing.T) {
	...
	fn := starlark.NewBuiltin("testfn", func(_ *starlark.Thread, _ *starlark.Builtin, _ starlark.Tuple, _ []starlark.Tuple) (starlark.Value, error) {
		callCount++
		if callCount <= 2 {
			return libkite.ResultOK(starlark.String("not ready")), nil
		}
		return libkite.ResultOK(starlark.String("ready")), nil
	})
...
func TestDoTimeout(t *testing.T) {
	...
	fn := starlark.NewBuiltin("slow", func(_ *starlark.Thread, _ *starlark.Builtin, _ starlark.Tuple, _ []starlark.Tuple) (starlark.Value, error) {
		...
		return libkite.ResultErr("still failing"), nil
	})
...
func TestDoResultReturn(t *testing.T) {
	thread := testThread()
	fn := makeFunc(libkite.ResultOK(starlark.MakeInt(42)))
...
func TestRetryResultTruth(t *testing.T) {
	...
	// ok result
	fn := makeFunc(libkite.ResultOK(starlark.None))
...
func TestRetryResultFreeze(t *testing.T) {
	rr := libkite.NewRetryResult(true, starlark.String("ok"), "", 1, time.Millisecond, nil)

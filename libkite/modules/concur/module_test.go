...
func TestChildThreadHasPermissions(t *testing.T) {
	...
	fn := makeBuiltin("check", func(childThread *starlark.Thread, args starlark.Tuple) (starlark.Value, error) {
		perms := libkite.GetPermissions(childThread)
		if perms == nil {
...
func TestChildThreadHasPrint(t *testing.T) {
	...
	reg := libkite.NewRegistry(&libkite.ModuleConfig{})
	reg.Register(New())
	rt, err := libkite.NewTrusted(
		nil,
		libkite.WithRegistry(reg),
		func(c *libkite.Config) {
...
func TestTryMapReturnsResult(t *testing.T) {
	...
	if !ok {
		t.Fatalf("expected *libkite.Result, got %T", result)
	}
	if !bool(r.Truth()) {
...
func TestMapOnErrorContinue(t *testing.T) {
	...
	// Item 0: ok
	r0 := list.Index(0).(*libkite.Result)
	if !bool(r0.Truth()) {
		t.Error("item 0 should be ok")
	}

	// Item 1: error
	r1 := list.Index(1).(*libkite.Result)
	if bool(r1.Truth()) {
		t.Error("item 1 should be error")
	}

	// Item 2: ok
	r2 := list.Index(2).(*libkite.Result)
	if !bool(r2.Truth()) {
...
func TestExecOnErrorContinue(t *testing.T) {
	...
	r0 := tuple[0].(*libkite.Result)
	if !bool(r0.Truth()) {
		t.Error("fn 0 should be ok")
	}
	r1 := tuple[1].(*libkite.Result)
	if bool(r1.Truth()) {
		t.Error("fn 1 should be error")
	}
	r2 := tuple[2].(*libkite.Result)
	if !bool(r2.Truth()) {

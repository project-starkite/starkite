...
const ModuleName libkite.ModuleName = "concur"
...
func newChildThread(parent *starlark.Thread, name string) *starlark.Thread {
	if rt := libkite.GetRuntime(parent); rt != nil {
	...
	if perms := libkite.GetPermissions(parent); perms != nil {
		libkite.SetPermissions(child, perms)
	}
	return child
}
	...
	if p.OnError == "continue" {
		...
		for i := 0; i < n; i++ {
			if errors[i] != nil {
				...
				wrapped[i] = libkite.ResultOK(results[i])
			}
		}
	...
	if p.OnError == "continue" {
		...
		for i := 0; i < n; i++ {
			if errors[i] != nil {
				...
				wrapped[i] = libkite.ResultOK(starlark.None)
			}
		}
		return starlark.NewList(wrapped), nil
	}

...
func buildToolFromSource(t *testing.T, src string, toolNames ...string) []*genai.Tool {
	...
	genaiMod := genai.New()
	globals, err := genaiMod.Load(&starbase.ModuleConfig{})
	if err != nil {
...
// newTestRuntime returns a minimal starbase.Runtime for handler tests.
func newTestRuntime(t *testing.T) *starbase.Runtime {
	t.Helper()

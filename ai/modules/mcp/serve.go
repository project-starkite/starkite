...
func (m *Module) serveBuiltin(thread *starlark.Thread, fn *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
	...
	if err := starbase.Check(thread, "mcp", "serve", p.Name); err != nil {
		return nil, err
	}
...
func buildServer(name, version string, tools []*genai.Tool, resources []*resourceEntry, prompts []*promptEntry, rt *starbase.Runtime) (*mcpsdk.Server, error) {
...
// graceful shutdown. Follows the pattern from starbase/modules/http/server.go.

...
func runScript(cmd *cobra.Command, args []string) error {
	...
	if _, err := os.Stat(scriptPath); os.IsNotExist(err) {
		return &starbase.ScriptError{
			Message:  fmt.Sprintf("script file not found: %s", scriptPath),
			ExitCode: starbase.ExitFileError,
		}
	}
	...
	if err := varStore.LoadDefaults(); err != nil {
		return &starbase.ScriptError{
			...
			ExitCode: starbase.ExitConfigError,
		}
	}
	...
	if err != nil {
		return &starbase.ScriptError{
			...
			ExitCode: starbase.ExitScriptError,
		}
	}
	defer rt.Cleanup()
	...
	if err := rt.Execute(ctx, string(content)); err != nil {
		...
		if _, ok := err.(*starbase.ScriptError); ok {
			return err
		}
		if _, ok := err.(*starbase.ExitError); ok {
			return err
		}

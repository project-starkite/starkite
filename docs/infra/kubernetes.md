...
Starkite's Kubernetes module interacts directly with the Kubernetes API to automate workloads, manage resources, and deploy custom controllers. By utilizing native Starlark bindings, you can inspect cluster state, apply resource manifests, run background reconciliation loops, and intercept API requests using admission webhooks—all running within a secure, sandboxed execution environment.
